    ...
    for job in jobs:
        ...
        if time_text:
            ...
            if age is not None and age > MAX_JOB_AGE_HOURS:
                print(f"    [SKIP] {job['title']} (posted {age}h ago) - stopping")
                break
            if age is None:
                print(f"    [WARN] Unrecognised time '{time_text}' for {job['title']}")
        else:
            print(f"    [WARN] No posting time for {job['title']}")
...
def scrape_all_keywords(keywords: list[str], batch_size: int = 1, on_new_job=None) -> None:
    ...
    - Rotates account at the END of each full keyword round (not mid-round).
    - If the current account fails login, runs the round without login
      (public LinkedIn search — no detail-panel time checks).
    - If all accounts are on cooldown (am.current is None), also runs without login.
    """
    am = _get_account_manager()
    print(am.status())

    account = am.current          # None = guest / no login
    account_idx = am.current_idx  # -1 = guest

    if account is None:
        print("  [LinkedIn] All accounts on cooldown — running as guest (no login).")
        driver = get_driver()
        logged_in = False
    else:
        ...
        if not logged_in:
            print(f"  [LinkedIn] Login failed for {account_name} — "
                  f"running this round without login.")
        else:
            print(f"  Active account: {account_name}")

    account_name = (
        account.get("name", f"Account {account_idx}")
        if account else "Guest"
    )
    ...
    while i < len(keywords):
        ...
        if challenge_triggered:
            # Close browser, continue rest of keywords without login
            reset_driver()
            os.environ["CHROME_PROFILE_SUFFIX"] = "li_guest"
            driver = get_driver()
            logged_in = False
            account_name = "Guest"
    ...
    # ── Recommended collections (only when logged in) ──────────────────
    if logged_in:
    ...
    am.record_used()
    reset_driver()   # close this session cleanly before next round opens
    am.rotate()
    print(am.status())